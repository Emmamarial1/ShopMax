...
def check_and_fix_database():
    """Check if database schema matches models and fix if needed"""
    with app.app_context():
        try:
            # Test if the orders table has the new columns
            from sqlalchemy import text
            conn = db.engine.connect()
            
            # Check if tracking_number column exists
            try:
                result = conn.execute(text("SELECT tracking_number FROM orders LIMIT 1"))
                print("✅ tracking_number column exists")
            except Exception as e:
                if "no such column" in str(e):
                    print("🔄 Adding missing columns to orders table...")
                    # Add the missing columns
                    conn.execute(text("ALTER TABLE orders ADD COLUMN tracking_number VARCHAR(100)"))
                    conn.execute(text("ALTER TABLE orders ADD COLUMN estimated_delivery DATETIME"))
                    conn.execute(text("ALTER TABLE orders ADD COLUMN actual_delivery DATETIME"))
                    print("✅ Missing columns added to orders table")
                else:
                    raise e
...
def products(category=None):
    ...
    if category:
        products = Product.query.options(joinedload(Product.seller)).filter_by(
            category=category, 
            is_active=True
        ).all()
    else:
        products = Product.query.options(joinedload(Product.seller)).filter_by(is_active=True).all()
    
    return render_template('products.html', items=products, category=category or 'all')
...
def seller_analytics():
    user = get_current_user()
    if user.user_type != 'seller' or not has_active_subscription(user):
        flash('Please subscribe to a plan to access analytics.', 'info')
        return redirect(url_for('seller_subscription'))
    
    # Get real analytics data with proper error handling
    analytics_data = get_seller_analytics_data(user.id)
    
    # Ensure all required fields are defined
    safe_analytics_data = {
        'total_revenue': analytics_data.get('total_revenue', 0),
        'total_orders': analytics_data.get('total_orders', 0),
        'products_sold': analytics_data.get('products_sold', 0),
        'store_rating': analytics_data.get('store_rating', 0),
        'reviews_count': analytics_data.get('reviews_count', 0),
        'revenue_change': analytics_data.get('revenue_change', 0),
        'orders_change': analytics_data.get('orders_change', 0),
        'products_change': analytics_data.get('products_change', 0),
        'recent_orders': analytics_data.get('recent_orders', []),
        'category_sales': analytics_data.get('category_sales', []),
        'top_products': analytics_data.get('top_products', []),
        'status_distribution': analytics_data.get('status_distribution', []),
        'line_chart_labels': analytics_data.get('line_chart_labels', []),
        'line_chart_data': analytics_data.get('line_chart_data', []),
        'current_week_revenue': analytics_data.get('current_week_revenue', 0)
    }
    
    return render_template('seller_analytics.html', 
                         user=user, 
                         analytics_data=safe_analytics_data)
